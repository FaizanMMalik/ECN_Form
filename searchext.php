...
{
	$result5=mysqli_query($conn,"Select ecnno from ecndetails WHERE jobno='$_GET[jobid]'");
}
...
?>
	...
	<body onload="highlightRequired() ; "  >
      ...
      <form action="">
	  <input type="text"  style="color: #FFFFFF; font-family: Verdana; font-weight: bold; font-size: 12px; background-color: #72A4D2; border: 3px solid #ccc;"placeholder="Enter Job no." name="jobid" value=<?php echo $_GET['jobid'];?>></input>

						<input type="Submit"  value="Search By ECN" class="align-center"  >
		...
		echo"<select name='eid' id='eid' style='  font-size: 12px; background-color: #72A4D2; border: 3px solid #ccc;'>";
		echo"<option style='font-weight: bold;color: #FFFFFF; font-family: Verdana;'>---ECN No---</option>";
		while($row=mysqli_fetch_array($result5)){
		echo"<option>$row[ecnno]";
		}
		echo"</select>"
		?> 
		<?php
...
{
	$sql=$sql." WHERE ecnno='$_GET[eid]'";
}

$resultecn = $conn->query($sql);
if ($resultecn->num_rows > 0) {
	if ($row = $resultecn->fetch_assoc()) {
	}
}
?>
		<input type="Submit"  value="GET ECN NUMBER" />
</form>
...
<center>
		...
		<table id="form-container">
				...
				$a=0;
				$b=0;
				$sql="SELECT t.name,t.id from ecntypeofchange e inner JOIN ecndetails d on e.ecn_id=d.ecnno INNER JOIN typeofchange t on t.id=e.typeOfChange_id WHERE d.ecnno=$_GET[eid]";
				...
				$sql="SELECT t.name,t.id from ecndepartmentaffected e inner JOIN ecndetails d on e.ecn_id=d.ecnno INNER JOIN departmentsaffected t on t.id=e.DepartmentAffected_id WHERE d.ecnno=$_GET[eid]";
				...
				<tr>
					...
					<td colspan="3">
						...
						<input type="text"
						       id="drawing-change" name="drawing-change" style="width:600px"
							   disabled
